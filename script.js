...
function handle_continent_btns(event){
    ...
    buildChart(labels,population,color);
}

async function handle_country_btn(event){
    let res = await fetchcities(event.target.textContent);
    //console.log(res);
    if (res.data) {
        ...
        buildChart(labels,population,"white");
        return;
    }
...
function buildChart(labels, cur_data,color) {
